...
/*기능 이니 4개씩 */
router.get('/bamboo', async (req, res, next) => {
  try {
    const posts = await db.BoardBamboo.findAll({
      include: [{
        model: db.Post,
      }]
    });
...
router.post('/bamboo', async (req, res, next) => {
  try {
    // 데이터는 req.body.type
    ...
    await newPost.addBoardBamboo({ // addBoardBamboo를 어떻게 정의해야 하는지...
      gomin_type: req.body.gomin_type,
    })
...
router.patch('/bamboo/:id', async (req, res, next) => {
  try {
...
router.delete('/bamboo/:id', async (req, res, next) => {
  try {
